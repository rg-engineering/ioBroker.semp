...
class Gateway {
	...
	EnableFastCharging(id, state) {
		const d = this.getDevice(id);
		if (d != null) {
			this.parentAdapter.log.debug("GW enable fast charging " + state);
	...
	SetMaxChargeTime(id, state) {
		const d = this.getDevice(id);
		if (d != null) {
			this.parentAdapter.log.debug("GW set max charge time " + state);
	...
	getAllDevices() {
		...
		try {
			...
			for (const d of this.devices.values()) {
				...
				const dev = this.getDevice(d.deviceInfo.Identification.DeviceId);
		...
		//this.parentAdapter.log.debug("got all devices " + JSON.stringify(ds));
