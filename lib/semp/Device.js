...
class Device {
                    ...
                    if (runToday) {
                        ...
                        if (this.getNoOfPlanningRequests() < 1) {
                            if (earliestStart > 0 || latestEnd > 0) {
                                ...
                                //later to reset at midnight? todo
                                this.CurrentOnTime = 0;
                            }
                        }
                        else {
    ...
    async sendEMRecommendation(em2dev) {
        ...
        StartCancelRequest(em2dev.DeviceControl.On);
        ...
        if (value) {
            if (this.device.TimerCancelIfNotOn != null && this.device.TimerCancelIfNotOn) {
                ...
                if (this.device.TimerCancelIfNotOnTime != null && Number(this.device.TimerCancelIfNotOnTime) > 0) {
                    ...
                    this.Gateway.parentAdapter.log.error(this.device.Name + " invalid time to cancel energy request " + JSON.stringify(this.device.TimerCancelIfNotOnTime));
                }
