 ...
 * State hinzufügen:
     warten auf Bedarfsanforderung
     warten Einschltzeitpunkt bei Bedarfsanforderun
     frühester Einschaltzeitpunkt erreicht, warten auf ein
     ein
     ein mit Überschuß

* wallbox -> Höhe der Energieanforderung = Batteriekapazität - geladene Kapazität
 * 
 * plug connected bei start holen
 * nicht genutzte time frames beim log markieren
...
class Device {
    ...
    constructor(gateway, device) {
        ...
        let planningrequestsSettings = {
            ...
            MaxPower: device.BatteryCapacity,
            MinPower: 0.1*device.BatteryCapacity

        }
        this.planningrequest = null;

        if (this.device.TimerActive) {
    ...
    async SwitchOff() {
        ...
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " turn device off");
    ...
    setLastPower(watts, minPower, maxPower) {
        ...
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " setLastPower " + watts + " " + typeof watts + " " + this.device.StatusDetection + " " + this.device.MeasurementUnit);
        ...
        if (this.device.StatusDetection == "FromPowerValue") {
            ...
            if (watts > limit) {
                if (typeof this.device.StatusDetectionLimitTimeOn != undefined && this.device.StatusDetectionLimitTimeOn != null && Number(this.device.StatusDetectionLimitTimeOn) > 0) {
                    ...
                    this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " status detection time limit is " + this.device.StatusDetectionLimitTimeOn + " going to on");
                    //going to on
                    if (this.deviceStatus.Status == "On" || this.StatusDetectionOnTimerID != null) {
                        ...
                        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " start setStatusOn - timer");
                    }
                    if (this.StatusDetectionOffTimerID) {
                        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " cancel setStatusOff - timer");
                ...
                if (!this.InMinRunTime) {
                    if (typeof this.device.StatusDetectionLimitTimeOff != undefined && this.device.StatusDetectionLimitTimeOff != null && Number(this.device.StatusDetectionLimitTimeOff) > 0) {
                        ...
                        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " status detection time limit is " + this.device.StatusDetectionLimitTimeOff + " going to off");
                        //going to off
                        if (this.deviceStatus.Status == "Off" || this.StatusDetectionOffTimerID != null) {
                            ...
                            this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " start setStatusOff - timer");
                        }
                        if (this.StatusDetectionOnTimerID) {
                            this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " cancel setStatusOn - timer");
                    ...
                    this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " still in min run time... not to switch off");
                }
            }
    ...
    async setRecommendationPowerConsumption(value) {
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " new recommendation power " + value);

        await SetWallboxPower(value);
    ...
    async Switch(value) {
        ...
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " got state " + JSON.stringify(curVal) + " target is " + value);
...
*/
    ...
    async SetState(state) {
        ...
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " set new state " + state);
    }
    ...
    async SetWallboxPower(value) {
        ...
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " set new charge power " + value);
    }
    ...
    setWallboxPlugConnected(state) {
        //set timeframe and request

        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " wallbox plug connected " + state);
    ...
    setWallboxIsCharging(state) {
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " wallbox charging " + state);
    ...
    setWallboxIsError( state) {
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " wallbox error " + state);
    }
