...
class Device {
    ...
    async checkRequests() {
        ...
        try {
            if (this.device.IsActive && this.device.TimerActive && this.device.EnergyRequestPeriods.length>0) {

                for (let r = 0; r < this.device.EnergyRequestPeriods.length; r++) {
                    ...
                    let start = this.device.EnergyRequestPeriods[r].EarliestStartTime.split(":");
                    let end = this.device.EnergyRequestPeriods[r].LatestEndTime.split(":");
                    let minRunTimes = this.device.EnergyRequestPeriods[r].MinRunTime.split(":");
                    let maxRunTimes = this.device.EnergyRequestPeriods[r].MaxRunTime.split(":");

                    let allchecked = true;
                    if (start.length != 2) {
                        this.Gateway.parentAdapter.log.error("unsupported time format " + this.device.EnergyRequestPeriods[r].EarliestStartTime + ", should be hh:mm");
                        allchecked = false;
                    }
                    if (end.length != 2) {
                        this.Gateway.parentAdapter.log.error("unsupported time format " + this.device.EnergyRequestPeriods[r].LatestEndTime + ", should be hh:mm");
                        allchecked = false;
                    }
                    if (minRunTimes.length != 2) {
                        this.Gateway.parentAdapter.log.error("unsupported time format " + this.device.EnergyRequestPeriods[r].MinRunTime + ", should be hh:mm");
                        allchecked = false;
                    }
                    if (maxRunTimes.length != 2) {
                        this.Gateway.parentAdapter.log.error("unsupported time format " + this.device.EnergyRequestPeriods[r].MaxRunTime + ", should be hh:mm");
                        allchecked = false;
                    }

                    //check days
                    this.Gateway.parentAdapter.log.debug("check run today (" + r + ") " + dayOfWeek + " " + JSON.stringify(this.device.EnergyRequestPeriods[r]));
                    let runToday = false;
                    if (this.device.EnergyRequestPeriods[r].Days=="everyDay") {
                        runToday = true;
                    }
                    else if (this.device.EnergyRequestPeriods[r].Days == "Monday" && dayOfWeek == 1) {
                        runToday = true;
                    }
                    else if (this.device.EnergyRequestPeriods[r].Days == "Tuesday" && dayOfWeek == 2) {
                        runToday = true;
                    }
                    else if (this.device.EnergyRequestPeriods[r].Days == "Wednesday" && dayOfWeek == 3) {
                        runToday = true;
                    }
                    else if (this.device.EnergyRequestPeriods[r].Days == "Thursday" && dayOfWeek == 4) {
                        runToday = true;
                    }
                    else if (this.device.EnergyRequestPeriods[r].Days == "Friday" && dayOfWeek == 5) {
                        runToday = true;
                    }
                    else if (this.device.EnergyRequestPeriods[r].Days == "Saturday" && dayOfWeek == 6) {
                        runToday = true;
                    }
                    else if (this.device.EnergyRequestPeriods[r].Days == "Sunday" && dayOfWeek == 0) {
                        runToday = true;
                    }
                    ...
                    if (allchecked) {
                        ...
                        if (runToday) {
                            ...
                            let CurrentOnTime = 0;
                            if (this.CurrentActivePeriod >= 0) {
                                CurrentOnTime = this.planningRequest.Timeframe[this.CurrentActivePeriod].CurrentOnTime;
                            }

                            if (earliestStart == 0 ) {
                                minRunTime = minRunTime - CurrentOnTime;
                                maxRunTime = maxRunTime - CurrentOnTime;
                            }
                            if (minRunTime < 0) minRunTime = 0;
                            if (maxRunTime < 0) maxRunTime = 0;

                            this.Gateway.parentAdapter.log.debug("Start " + StartTime.toLocaleTimeString() + " End " + EndTime.toLocaleTimeString() + " earliest: " + earliestStart + " latest: " + latestEnd + " current on time: " + CurrentOnTime);
                            ...
                            //find right request in list
                           
                            if (!this.PlanningRequestsExist(r)) {
                                if (earliestStart > 0 || latestEnd > 0) {

                                    

                                    this.addPlanningRequest(r,earliestStart, latestEnd, minRunTime, maxRunTime);

                                    //otherwise if device was on before we get min running time of 0
                                    
                                    //this.CurrentOnTime = 0;
                                }
                            }
                            else {
                                //let reqId = r; //we use the same order as in request list

                                if (earliestStart == 0 && latestEnd == 0) {

                                    this.clearPlanningRequest(r); //remove 
                                    //this.RemainingMaxOnTime = 0;
                                    //this.CurrentOnTime = 0;

                                    if (this.device.SwitchOffAtEndOfTimer) {
                                        await this.SwitchOff();
                                    }
                                }
                                else {
                                    this.updatePlanningRequest(r, earliestStart, latestEnd, minRunTime, maxRunTime);
                                }
                            }
                        }
                        else {
                ...
                this.Gateway.parentAdapter.log.debug("not active or no energy request in list");
            }
        }
    ...
    async UpdateTimes() {
        ...
        if (this.deviceStatus.Status=="On") {
            this.planningRequest.Timeframe[this.CurrentActivePeriod].CurrentOnTime = this.planningRequest.Timeframe[this.CurrentActivePeriod].CurrentOnTime +60;
        }
        ...
        //=============================================================
        let Hour = Math.floor(CurrentOnTime / 60 / 60);
        let Minutes = Math.floor((CurrentOnTime - (Hour*60*60)) / 60); 
        let sHour = "0";
        if (Hour < 10) {
            sHour = "0" + Hour;
        }
        ...
        let RemainingMaxOnTime = 0;
        if (this.CurrentActivePeriod >= 0) {
            RemainingMaxOnTime = this.planningRequest.Timeframe[this.CurrentActivePeriod].RemainingMaxOnTime;
        }
        Hour = Math.floor(RemainingMaxOnTime / 60 / 60);
        Minutes = Math.floor((RemainingMaxOnTime - (Hour*60*60)) / 60);
        sHour = "0";
        if (Hour < 10) {
            sHour = "0" + Hour;
        }
    ...
    updatePlanningRequest(timeframeId, earliestStart, latestEnd, minRunTime, maxRunTime) {
        ...
        //update the one with correct id

        for (let t = 0; t < this.planningRequest.Timeframe.length; t++) {
            if (this.planningRequest.Timeframe[t].TimeframeId == timeframeId) {

                //xxx maxontime und on time pro periode (auch so in DP's')

                this.planningRequest.Timeframe[t].EarliestStart = earliestStart;
                this.planningRequest.Timeframe[t].LatestEnd = latestEnd;
                this.planningRequest.Timeframe[t].MinRunningTime = minRunTime;
                this.planningRequest.Timeframe[t].MaxRunningTime = maxRunTime;

                if (earliestStart == 0 && latestEnd > 0) {
                    this.CurrentActivePeriod = timeframeId;
                    this.planningRequest.Timeframe[t].RemainingMaxOnTime = latestEnd;
                }
                else {
                    this.planningRequest.Timeframe[t].RemainingMaxOnTime = 0;
                }
            }
        }

        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " updatePlanningRequest done " + JSON.stringify(this.planningRequest));
 
    }

    PlanningRequestsExist(timeframeId) {

        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " PlanningRequestsExist  " + timeframeId);

        let bRet = false;
        for (let t = 0; t < this.planningRequest.Timeframe.length; t++) {
            if (this.planningRequest.Timeframe[t].TimeframeId == timeframeId) {
                this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " PlanningRequestsExist found it" );
                bRet = true;
            }
        }
    ...
    clearPlanningRequest(timeframeId) {

        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " clearPlanningRequest before " + JSON.stringify(this.planningRequest.Timeframe));

        for (let t = 0; t < this.planningRequest.Timeframe.length; t++) {
            if (this.planningRequest.Timeframe[t].TimeframeId == timeframeId) {
                this.planningRequest.Timeframe.splice(t, 1);
            }
        }
        this.Gateway.parentAdapter.log.debug(this.deviceInfo.Identification.DeviceId + " clearPlanningRequest after " + JSON.stringify(this.planningRequest.Timeframe));
    ...
    */
    ...
    CancelRequest() {
        ...
        this.Gateway.parentAdapter.log.debug(this.device.Name + "cancel energy request + " + this.CurrentActivePeriod + " because device is not switched on");

        //welcher?? -> activer
        this.clearPlanningRequest(this.CurrentActivePeriod);
    ...
    async getCurrentStates() {
        ...
        try {
            ...
            if (curVal != null) {
                ...
                if (vals.length > 1) {
                    if (this.CurrentActivePeriod >= 0) {
                        this.planningRequest.Timeframe[this.CurrentActivePeriod].CurrentOnTime = (vals[0] * 60 * 60) + (vals[1] * 60);
                    }
                }
            }
