...
class Device {
				...
				if (this.device.StatusDetectionLimitTimeOn !== undefined && this.device.StatusDetectionLimitTimeOn != null && Number(this.device.StatusDetectionLimitTimeOn) > 0) {
					...
					if (this.deviceStatus.Status == "On" || this.StatusDetectionOnTimerID != null) {
						...
						if (this.DishwasherSwitchState != "On") {
							this.DishwasherSwitchState = "On";
							this.Gateway.parentAdapter.log.debug(this.device.Name + " set DishwasherSwitchState to On!!!!");
						}
					}
					else {
					...
					if (this.device.StatusDetectionLimitTimeOff !== undefined && this.device.StatusDetectionLimitTimeOff != null && Number(this.device.StatusDetectionLimitTimeOff) > 0) {
						...
						if (this.deviceStatus.Status == "Off" || this.StatusDetectionOffTimerID != null) {
							...
							if (this.DishwasherSwitchState != "Off") {
								this.DishwasherSwitchState = "Off";
								this.Gateway.parentAdapter.log.debug(this.device.Name + " set DishwasherSwitchState to Off!!!!");
							}
						}
						else {
	...
	setOnOff(state) {
		...
		if (this.DishWasherMode) {
			...
			this.Gateway.parentAdapter.log.debug(this.device.Name + " set new DishwasherSwitchState to " + state);
	...
	async DishWasherSequence() {
		...
		switch (this.dishwasherstate) {
			...
			case this.states.off:
				...
				if (this.DishwasherSwitchState == "On") {
					...
					if (this.planningrequest != null) {
						this.planningrequest.SetDeviceStatus("Off");
					}
					...
					if (this.InMinRunTime) {
						...
						//sollte hier null sein...
						if (current != null && current.val != null) {
							this.setLastPower(current.val, 0, 0);
							// OFF timer sollte nicht starten, da wir ja schon aus waren
						}
			...
			case this.states.waiting4OnRecommendation:
				...
				if (this.DishWasherRecommendation) {
					...
					if (this.planningrequest != null) {
						this.planningrequest.SetDeviceStatus("On");
					}

					this.dishwasherstate = this.states.waiting4On;

					//keinen timeout abwarten, weil wir sonst direkt nach off durchlaufen
					this.DishwasherSwitchState = "On";
				...
				if (this.planningrequest != null && this.planningrequest.getAllTimeframesFinished()) {
					this.Gateway.parentAdapter.log.debug(this.device.Name + " device will not turn on today");
					this.dishwasherstate = this.states.off;
			...
			/*
			case this.states.TFended:
				break
			case this.states.waiting: 
				break;
			case this.states.FinishedWaiting4TF:
				break;
			case this.states.TFendedWaiting4Device:
				break;
			case this.states.DeviceFinished: 
				break;
			case this.states.DeviceInStandby:
				break;
				*/
			default:
		...
		this.Gateway.parentAdapter.log.debug(this.device.Name + " DishWasherSequence: dishwasherstate: " + this.dishwasherstate + " DishwasherSwitchState: " + this.DishwasherSwitchState + " status " + this.deviceStatus.Status);
		...
		if (this.dishwasherstate == this.states.off) {
			...
			//wait until user switches device on
			if (this.DishwasherSwitchState == "On") {
			...
			//todo: falls timeframe hier hier aktiv wird, -> deaktivieren
			...
			todo activate timeframe if not active
		...
		if (lastDishwasherState != this.dishwasherstate) {
			...
			this.Gateway.parentAdapter.log.debug(this.device.Name + " new dishwasher state " + this.dishwasherstate + " DishWasherRecommendation: " + this.DishWasherRecommendation);
		}
...
*/
	...
	async SetState() {
		...
		else if (this.DishWasherMode) {

			//just store the state
			state = this.DishwasherSwitchState;
		}
		else {
