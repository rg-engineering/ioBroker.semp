...
DeviceStatusDetectionLimitTime      devices[id].StatusDetectionLimitTime
    ...
    */
            ...
            if (this.device.IsActive && this.device.TimerActive) {
                ...
                if (allchecked) {
                    ...
                    if (runToday) {
                        ...
                        //now reduce it by already use OnTime
                        ...
                        this.Gateway.parentAdapter.log.debug("Start " + StartTime.toLocaleTimeString() + " End " + EndTime.toLocaleTimeString() + " earliest " + earliestStart + " latest " + latestEnd);
    ...
    setLastPower(watts, minPower, maxPower) {
        ...
        if (this.device.StatusDetection == "FromPowerValue") {
            ...
            if (typeof this.device.StatusDetectionLimitTime != undefined && this.device.StatusDetectionLimitTime != null && Number(this.device.StatusDetectionLimitTime) > 0) {
                

                if (watts > limit) {
                    this.Gateway.parentAdapter.log.debug("status detection time limit is " + this.device.StatusDetectionLimitTime + " going to on");
                    //going to on
                    if (this.deviceStatus.Status == "On" || this.StatusDetectionOnTimerID != null) {
                        //nothing to do, already true or timer started
                    }
                    else {
                        this.StatusDetectionOnTimerID = setTimeout(this.SetStatusOn.bind(this), this.device.StatusDetectionLimitTime * 60 * 1000);
                    ...
                    this.Gateway.parentAdapter.log.debug("status detection time limit is " + this.device.StatusDetectionLimitTime + " going to off");
                    //going to off
                    if (this.deviceStatus.Status == "Off" || this.StatusDetectionOffTimerID != null) {
                        //nothing to do, already false or timer started
                    }
                    else {
                        this.StatusDetectionOffTimerID = setTimeout(this.SetStatusOff.bind(this), this.device.StatusDetectionLimitTime * 60 * 1000);
    ...
    async setRecommendation(value) {
        ...
        if (this.device.HasOIDSwitch) {
            ...
            if (curVal != null && curVal.val != value) {
                ...
                this.Gateway.parentAdapter.setForeignState(this.device.OID_Switch, value)
            }
        }
    }
