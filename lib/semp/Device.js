...
class Device {
				...
				if (!this.InMinRunTime) {
					if (this.device.StatusDetectionLimitTimeOff !== undefined && this.device.StatusDetectionLimitTimeOff != null && Number(this.device.StatusDetectionLimitTimeOff) > 0) {
						...
						if (this.deviceStatus.Status == "Off" || this.StatusDetectionOffTimerID != null) {
							...
							this.Gateway.parentAdapter.log.debug(this.device.Name + " already off, no timer start");
						}
						else {
	...
	DishWasherSequence() {
		...
		this.Gateway.parentAdapter.log.debug(this.device.Name + " DishWasherSequence: dishwasherstate: " + this.dishwasherstate + " DishwasherSwitchState: " + this.DishwasherSwitchState);
