...
/*
todo
* request canceln, wenn gerät  oder schon wieder aus 
 ...
 * 
 ...
 min/MaxEnergy einstellbar per DP
   logging für energy -> dateiname??
   rolling logging per day for csv

wallbox: recommended power als DP


 * nicht genutzte time frames beim log markieren

-> State  On obwohl recommendation false 
...
class Device {
    ...
    constructor(gateway,
        ...
        this.LogToCSVPath = LogToCSVPath;

        this.createCsvWriter = null;
        this.csvWriter = null;
        this.StartCSVLog();
    ...
    StartCSVLog() {
        try {
            if (this.LogToCSV) {
                ...
                //check that path and file are valid!!
                var fs = require('fs');
                var stats = fs.statSync(this.LogToCSVPath);
                let isValid = false;
                if (stats.isFile()) {
                    //nothing to do
                    isValid = true;
                    this.Gateway.parentAdapter.log.debug("valid file for csv logging " + this.LogToCSVPath);
                }
                else if (stats.isDirectory()) {
                    //add file name

                    let name = "Energy" + this.device.ID;

                    this.LogToCSVPath = this.LogToCSVPath + "/" + name + ".csv";
                    isValid = true;
                    this.Gateway.parentAdapter.log.debug("valid path for csv logging (with new file name) " + this.LogToCSVPath);
                }
                else {
                    this.Gateway.parentAdapter.log.error("invalid path / file for csv logging " + this.LogToCSVPath);
                }



                if (isValid) {
                    this.Gateway.parentAdapter.log.info("start logging to " + this.LogToCSVPath);

                    this.createCsvWriter = require('csv-writer').createObjectCsvWriter;

                    this.csvWriter = this.createCsvWriter({
                        path: this.LogToCSVPath,
                        header: [
                            { id: 'Time', title: 'Time' },
            ...
            this.Gateway.parentAdapter.log.error("exception in StartCSVLog [" + e + "]");
        }
    }

    async WriteCSVLog(records) {

        try {
            if (this.LogToCSV) {
                this.Gateway.parentAdapter.log.info("write to csv " + JSON.stringify(records));
                await this.csvWriter.writeRecords(records);
            }
        }
        catch (e) {
            this.Gateway.parentAdapter.log.error("exception in WriteCSVLog [" + e + "]");
        }
    }
