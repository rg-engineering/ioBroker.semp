...
class Device {
    ...
    constructor(gateway,
        ...
        let planningrequestsSettings = {
            ...
            WallboxChargeTime: device.WallboxChargeTime
        }
        ...
        if (device.ID === undefined || device.ID.length < 25) {
            this.Gateway.parentAdapter.log.error(this.device.Name + " wrong device id " + device.ID + "! must follow F-xxxxxxxx-yyyyyyyyyyyy-zz");
        }


        this.Gateway.parentAdapter.log.debug(this.device.Name + " check DeviceID " + device.ID + " type " + typeof device.ID);

        let ids = device.ID.split("-");
        ...
        if (device.Name === undefined || device.Name.length < 2) {
            this.Gateway.parentAdapter.log.error(this.device.Name + " wrong device name " + device.Name);
        }

        if (device.Type === undefined || device.Type==null) {
            this.Gateway.parentAdapter.log.error(this.device.Name + " wrong device type " + device.Type);
        }

        if (device.MeasurementMethod === undefined || device.MeasurementMethod == null) {
            this.Gateway.parentAdapter.log.error(this.device.Name + " wrong device MeasurementMethod " + device.MeasurementMethod);
        }

        if (device.SerialNr === undefined || device.SerialNr.length < 2) {
            this.Gateway.parentAdapter.log.error(this.device.Name + " wrong device SerialNr " + device.SerialNr);
        }
        if (device.MaxPower === undefined || device.MaxPower == null || device.MaxPower.length < 1) {
            this.Gateway.parentAdapter.log.error(this.device.Name + " max. Power not set!  " + device.MaxPower);
            device.MaxPower = 100;
        }
    ...
    Check2Switch() {
        ...
        if (this.planningrequest != null) {
            let ret = this.planningrequest.Check2Switch();
...
*/
    ...
    async SetState() {
        ...
        let currentState = await this.Gateway.parentAdapter.getStateAsync(key);
    ...
    */
    ...
    async setStateTypebased(actor) {
        if (actor !== undefined && actor != null) {
            ...
            if (actor.Type == "Boolean") {
                ...
                if (typeof url === 'string' && url != null && url.length > 5) {
                    ...
                    try {
                        ...
                        adapter.log.debug(actor.Name + " call get URL " + url);

                        result = await axios.get(url, null, config);
                    }

                    catch (error) {
                        adapter.log.error(actor.Name + " got error " + JSON.stringify(error) + " after calling url");
                    }
            ...
            this.Gateway.parentAdapter.log.error("setStateTypeBased: actor not found");
        }
        return;
    }
    ...
    SetMaxChargeTime(state) {
        ...
        if (this.planningrequest != null) {
            let ret = this.planningrequest.SetMaxChargeTime(state);
    ...
    async GetMaxChargTime() {
        ...
        if (this.device.WallboxChargeTime != null && this.device.WallboxChargeTime == 4) {
            ...
            let key = "Devices." + this.device.Name + ".MaxChargeTime";
            let curVal = await this.Gateway.parentAdapter.getStateAsync(key);
