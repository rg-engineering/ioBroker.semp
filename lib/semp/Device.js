...
*/
	...
	WallboxSubscribeUrl() {
		for (let i = 0; i < this.URLs2Check.length; i++) {
			...
			this.UrlTimerId[i] = setInterval(this.CheckURLStatus.bind(this), PollRate*1000 + i*100, this.URLs2Check );
		}
	}
	...
	async CheckURLStatus(url) {
		...
		try {
			...
			this.Gateway.parentAdapter.log.debug(this.device.Name + " call get URL " + url);
