					...
					if (this.device.WallboxOIDs[o].active) {
						if (this.device.WallboxOIDs[o].Name == "DeviceOIDPlugConnected") {
							DeviceOIDPlugConnected = {
								...
								SetValue: this.device.WallboxOIDs[o].Type == "URL" ? this.device.WallboxOIDs[o].OID : this.device.WallboxOIDs[o].SetValue,
...
*/
			...
			if (actor.Type == "Boolean") {
				...
				if (typeof url === "string" && url != null && url.length > 5) {
					...
					try {
						...
						this.Gateway.parentAdapter.log.debug(actor.Name + " call get URL " + url);

						const result = await axios.get(url, null, config);

						this.Gateway.parentAdapter.log.debug("result " + JSON.stringify(result));
					}

					catch (error) {
						this.Gateway.parentAdapter.log.error(actor.Name + " got error " + JSON.stringify(error) + " after calling url");
					}
	...
	checkStateTypebased(sensor, value) {
		...
		if (sensor !== undefined && sensor != null) {
			...
			this.Gateway.parentAdapter.log.debug(sensor.Name + " sensor type " + sensor.Type + " steValue " + sensor.SetValue + " value " + value + " " + typeof sensor.SetValue + " " + typeof value);
			if (sensor.Type == "Boolean") {
